...
function registerCommonVariables() {
	...
	events = function (num) { return this['scripts.ActorEvents_' + num] }.bind(this);
}
...
function registerCheat(command, fn, message) { cheats[command] = { fn: fn, message: message } };
...
function registerCheats(cheatMap, higherKeys = []) {
	...
	let fn = function (params) {
		...
		if (cheatMap.hasOwnProperty("fn")) return cheatMap.fn.call(this, higherKeys.concat(cheatMap.name).concat(params).splice(1));

		if (params.length > 0) {
			if (!cheatMap.hasOwnProperty('configurable'))
				return `Wrong subcommand, use one of these:\n` + cheatMap.subcheats.map(p => `${cmd} ${p.name} (${p.message})`).join("\n");

			let config = higherKeys.reduce((obj, key) => obj[key], cheatConfig);

			let val = params.slice(cheatMap.configurable['isObject'] ? 1 : 0).join(' ');
			if (val === '') return `Invalid value, must be a boolean, number or function that returns a number.`;
			try {
				val = JSON.parse(val);
			} catch (e) {
				val = new Function('return ' + val)();
				if(isNaN(val(1))) return `Invalid value, must be a boolean, number or function that returns a number.`;
			}
		...
		if (!params[0] && cheatMap['subcheats']) {
			for(const i in stateObject[cheatMap.name]) stateObject[cheatMap.name][i] = !stateObject[cheatMap.name + 's'];
...
registerCheat('cheats', function (params) {
	let cheatsAvailable = [];
...
const minigameCheat = function (params) {
	...
	return `${cheatState.minigame[params[0]] ? 'Activated' : 'Deactivated'} ${params[0]} minigame cheat.`;
}
registerCheats({
	name: 'minigame',
...
registerCheats({
	...
	subcheats: [
		{
			name: 'gembuylimit', fn: function (params) {
        ...
        { name: 'plunderous', message: '100% plunderous mob spawn rate', configurable: { isObject: true } },
		{ name: 'candy', message: 'enable candy use everywhere' },
		{ name: 'eventitems', message: 'unlimited event item drops' },
		{ name: 'autoloot', message: 'autoloot immeditely to inventory. Optionally add config to move directly to your chest.', configurable: { isObject: true } },
		{name: 'perfectobols', message: 'Roll all obols perfectly for class. Family and inventory obols update on character change.', fn: function (params) {
				if (!cheatState.wide[params[0]]) rollAllObols();
				cheatState.wide[params[0]] = !cheatState.wide[params[0]];
				return `${cheatState.wide[params[0]] ? 'Activated' : 'Deactived'} Perfect obol rolls. Family and inventory obols update on character change.`;
			}
		},
...
registerCheats({
	...
	subcheats: [
		...
		{ name: 'smith', message: 'smithing cost nullification (change maps to have the effect apply).' },
		{ name: 'companion', message: 'Enab;e companion', configurable: true}
	]
});
...
registerCheats({
	...
	subcheats: [
		...
		{ name: 'petrng', message: 'max strength pets (for level and egg, with a tiny bit of randomness).' },
		...
		{ name: 'mainframe', message: 'mainframe cheats', configurable: { isObject: true } },
		{ name: 'chipbonuses', message: 'chip bonuses', configurable: { isObject: true } },
		{ name: 'meals', message: 'meal bonus cheats', configurable: { isObject: true } },
	],
});
...
registerCheats({
	...
	subcheats: [
		...
		{ name: 'ability', message: 'zero ability cooldown, mana cost nullification and cast time 0.1s.' },
		...
		{
			name: 'speed', message: 'weapon super speed', fn: function (params) {
		...
		{
			name: 'card', message: 'Efaunt, Chaotic Efaunt, Dr Defecaus, Oak Tree and Copper are altered with insane stats', fn: function (params) {
...
registerCheat('nomore', function (params) {
	if (params && params[0] && (regex = new RegExp(params[0])) && Object.keys(itemDefs).filter(item => regex.test(item)).length > 0) {
		cheatConfig.nomore.items.map(r => r.toString()).includes(regex.toString()) ?
			cheatConfig.nomore.items.splice(cheatConfig.nomore.items.map(r => r.toString()).indexOf(regex.toString()), 1) :
...
registerCheats({
	...
	subcheats: [
		...
		{
			name: 'use', message: 'Upgrade stone doesn\'t use a slot (risky)', fn: function (params) {
				for (const [index, element] of Object.entries(itemDefs))
...
registerCheats({
	name: 'multiply',
	message: 'Multiplies stats by the number (or applies function) given. use reasonably, ridiculous numbers can cause shadowbanning!',
	subcheats: [
		{
			name: 'damage', message: 'Multiplies damage by the number given (use reasonably!)',
			configurable: { valueTransformer: val => !isNaN(val) ? (new Function(`t => t * ${val}`))() : val }
		},
		{
			name: 'efficiency', message: 'Multiplies skill efficiency by the number given (use reasonably!)',
			configurable: { valueTransformer: val => !isNaN(val) ? (new Function(`t => t * ${val}`))() : val }
		},
		{
			name: 'afk', message: 'Multiplies AFK % by the number given (use reasonably!)',
			configurable: { valueTransformer: val => !isNaN(val) ? (new Function(`t => t * ${val}`))() : val }
		},
		{
			name: 'drop', message: 'Multiplies drop rate by the number given (use reasonably!)',
			configurable: { valueTransformer: val => !isNaN(val) ? (new Function(`t => t * ${val}`))() : val }
		},
		{
		...
		{
			name: 'monsters', message: 'Multiplies the number of monsters on the map by the number given', configurable: true
		}
	],
});
...
registerCheats({
	...
	subcheats: [
		{ name: 'save', message: 'Saves the current values of items and cards', fn: function (params) {
		...
		{ name: 'item', message: 'Restores original item values.', fn: function (params) {
			itemDefs = dictVals.itemDefs;
			return `Restored original item values.`;
		} },
		{ name: 'card', message: 'Restores original card values.', fn: function (params) {
...
registerCheats({
	...
	subcheats: [
		{
			name: 'inv', message: 'Change the quantity of the first inventory slot to this value', fn: function (params) {
		...
		{
			name: 'chest', message: 'Change the quantity of the first chest slot to this value', fn: function (params) {
				const setqnty = params[1] || 1;
...
registerCheat('jackpot', function () {
	(function () {
...
registerCheat('fire', function (params) {
	(function () {
		this._TRIGGEREDtext = params[0]  || "a6";
		this._customEvent_PachiStuff2();
	}).bind(bEngine.gameAttributes.h.PixelHelperActor[21].behaviors.behaviors[0].script)();
		return "JACKPOT!!!";
	},
...
registerCheats({
	...
	subcheats: [
		{
			name: 'item', message: 'Search for an item', fn: function (params) {
		...
		{
			name: 'monster', message: 'Search for a monster', fn: function (params) {
				const queryX = params.slice(1) && params.slice(1).length ? params.slice(1).join(' ').toLowerCase() : undefined;
		...
		{
			name: 'talent', message: 'Search for a talent', fn: function (params) {
				const queryX = params.slice(1) && params.slice(1).length ? params.slice(1).join(' ').toLowerCase() : undefined;
		...
		{
			name: 'smith', message: 'Search for an item to smith', fn: function (params) {
				const queryX = params.slice(1) && params.slice(1).length ? params.slice(1).join(' ').toLowerCase() : undefined;
				const ItemVals = [[], []]
...
registerCheats({
	...
	subcheats: [
		{ name: 'item', message: 'list items. third param optional filter', fn: listFunction },
		{ name: 'monster', message: 'list monsters. third param optional filter', fn: listFunction },
		{ name: 'class', message: 'list classes. third param optional filter', fn: listFunction },
		{ name: 'card', message: 'list classes. third param optional filter', fn: listFunction },
		{ name: 'quest', message: 'list quests. third param optional filter', fn: listFunction },
		{ name: 'map', message: 'list maps. third param optional filter', fn: listFunction },
		{ name: 'talent', message: 'list talents. third param optional filter', fn: listFunction },
		{ name: 'ability', message: 'list abilities. third param optional filter', fn: listFunction },
		{ name: 'smith', message: 'list smithing recipes. third param optional filter', fn: listFunction },
		{ name: 'gga', message: 'list game attributes. third param optional filter', fn: listFunction },
	]
});
	...
	if (params[0] === "inv") {
		...
		if(parseInt(params[1])) {
			...
			bEngine.gameAttributes.h.CogOrder.forEach(
				(v, k) => (
					typeof v === 'string' && (k > (100 + bEngine.gameAttributes.h.CogOrder.slice(100).toString().match(/Player/g).length - 1 + parseInt(cheatConfig.wipe.cogs))) && !v.includes("Player") && (
						bEngine.gameAttributes.h.CogOrder[k] = "Blank",
						bEngine.gameAttributes.h.CogMap[k].keys().keys.forEach(a => delete (bEngine.gameAttributes.h.CogMap[k].h[a]))
					)
				)
			);
		}
...
registerCheats({
	...
	subcheats: [
		...
		{ name: "overpurchases", message: 'Set all overpurchased items in the gem shop to their max safe value.', fn: wipeFunction },
...
registerCheats({
	...
	subcheats: [
		...
		{
			name: 'furnace', message: '!danger! Change all furnace lvl to this value', fn: function (params) {
				const lvltype = params[0];
				const setlvl = parseInt(params[1]) || -1;
				if (setlvl == -1) return `The lvl value has to be numeric!`; // Yup this is a dummy-proof measurement to prevent account bricking
		...
		{
			name: 'statue', message: '!danger! Change all statue lvl to this value', fn: function (params) {
				const setlvl = parseInt(params[1]) || -1;
				if (setlvl == -1) return `The lvl value has to be numeric!`; // Yup this is a dummy-proof measurement to prevent account bricking
		...
		{
			name: 'anvil', message: '!danger! Change all anvil lvl to this value', fn: function (params) {
				const setlvl = parseInt(params[1]) || -1;
				if (setlvl == -1) return `The lvl value has to be numeric!`; // Yup this is a dummy-proof measurement to prevent account bricking
		...
		{
			name: 'talent', message: '!danger! Change all talent lvls to this value', fn: function (params) {
				const setlvl = parseInt(params[1]) || -1;
				if (setlvl == -1) return `The lvl value has to be numeric!`; // Yup this is a dummy-proof measurement to prevent account bricking
		...
		{
			name: 'stamp', message: 'Change all stamp lvls to this value', fn: function (params) {
				const setlvl = parseInt(params[1]) || -1;
				if (setlvl == -1) return `The lvl value has to be numeric!`; // Yup this is a dummy-proof measurement to prevent account bricking
		...
		{
			name: 'shrine', message: '!danger! Change all shrine lvls to this value', fn: function (params) {
				const setlvl = parseInt(params[1]) || -1;
				if (setlvl == -1) return `The lvl value has to be numeric!`; // Yup this is a dummy-proof measurement to prevent account bricking
...
const alchFn = function (params) {
	const alchdict = { "orangebubbles": 0, "greenbubbles": 1, "purplebubbles": 2, "yellowbubbles": 3, "vials": 4, "color": 5, "liquids": 6, "cauldrons": 8 };
...
async function setup() {
	...
	rtn.push("Cheats will find as you type, so if you're looking for eg gem cheats, or an item just type it and see what comes up");
	rtn.push("--------------------");
	return rtn.join("\n");
}
...
function setupBehaviorScriptProxies() {
	...
	behavior.runPeriodically = new Proxy(behavior.runPeriodically, {
		apply: function (originalFn, context, argumentsList) {
			try {
				if (cheatState.godlike.poison && argumentsList[0] == 2e3 &&	argumentsList[2]['behaviors']['behaviors'][0]['name'] == "ActorEvents_575") {
						argumentsList[0] = 5;
				}
			} catch (e) {}
...
function setupAutoLootProxy() {
	...
	actorEvents44.prototype.init = new Proxy(actorEvents44.prototype.init, {
		apply: function (originalFn, context, argumentsList) {
			...
			if (
				...
				(/.*(LOG|ORE|LEAF|FISH|BUG|CRITTER|SOUL|FOOD|STATUE|TELEPORT|FISHING_ACCESSORY|OFFICE_PEN|BOSS_KEY|FRAGMENT|UPGRADE|MONSTER_DROP|MATERIAL|CARD).*/i
    ...
    events(34).prototype._event_ItemGet = new Proxy(events(34).prototype._event_ItemGet, {
        apply: function (originalFn, context, argumentsList) {
			return cheatState.wide.autoloot && cheatConfig.wide.autoloot.hidenotifications && [0, 1].includes(context._Deployment) ?
	...
	this.React.createElement = new Proxy(this.React.createElement, {
		apply: function (originalFn, context, argumentsList) {
			if (cheatState.w1.companion && argumentsList[0].includes("Companion")) {
				if (["deleteCompanion", "swapCompanionOrder"].includes(argumentsList[0])) return new Promise(resolve => resolve(1));
...
function setupMonsterProxy() {
	bEngine.setGameAttribute("MonsterRespawnTime", new Proxy(bEngine.getGameAttribute("MonsterRespawnTime"), {
		set: function (obj, prop, value) {
			return obj[prop] = cheatState.godlike.respawn ? cheatConfig.godlike.respawn(value) : value;
		}
	}));
...
function setupOptionsListAccountProxy() {
	...
	optionsListAccount._88 = optionsListAccount[88];
	Object.defineProperty(optionsListAccount, 88, {
		get: function () {
			return cheatState.w4.arena ? 1 : this._88;
		},
		set: function (value) {
			this._88 = value;
			return true;
...
function setupArbitraryProxy(){
	...
	ActorEvents12._customBlock_SkillStats = function(...argumentsList) {
		...
		if (cheatState.multiply.efficiency && t.includes("Efficiency"))	return Reflect.apply(SkillStats, this, argumentsList) * cheatConfig.multiply.efficiency;
	...
	ActorEvents12._customBlock_skillstats2 = function(...argumentsList) {
		const t = argumentsList[0];
		if (cheatState.w3.worshipspeed && t == "WorshipSpeed") return 5000; // 1000 worship%/h
		if (cheatState.multiply.efficiency && t.includes("Efficiency"))	return Reflect.apply(SkillStats2, this, argumentsList) * cheatConfig.multiply.efficiency;
	...
	ActorEvents12._customBlock_TotalStats = (...argumentsList) => {
		return Reflect.apply(TotalStats, this, argumentsList) * ((cheatState.multiply.drop && argumentsList[0] == "Drop_Rarity") ? cheatConfig.multiply.drop : 1);
	};
	...
	ActorEvents12._customBlock_GenerateMonsterDrops = function(...argumentsList) {
			...
			drops = drops.filter(drop => !cheatConfig.nomore.items.some(regex =>
				regex.test(drop[0])
			));
...
function setupAFKRateProxy() {
	events(124)._customBlock_AFKgainrates = new Proxy(events(124)._customBlock_AFKgainrates, {
		apply: (originalFn, context, argumentsList) => {
			if (cheatState.multiply.afk) return Reflect.apply(originalFn, context, argumentsList) * cheatConfig.multiply.afk;
			return Reflect.apply(originalFn, context, argumentsList);
		}
	});
}
...
function setupMonsterKillProxy() {
	...
	events(124)._customBlock_MonsterKill = (...argumentsList) => {
		...
		if (cheatState.wide.plunderous &&
			...
			!bEngine.getGameAttribute("CustomLists").h.NonAFKmonsters.includes(e.getValue("ActorEvents_1", "_MonsterType")) &&
...
function setupAnvilProxy() {
	...
	events(189)._customBlock_AnvilProduceStats = function(...argumentsList) {
		if (cheatState.w1.anvil) {
			...
			if (t == "ProductionSpeed") return cheatConfig.w1.anvil.productionspeed(Reflect.apply(anvilProduceStats, this, argumentsList));
		}
...
function updateCListFuncDict() {
	...
	CListFuncDict = {
		...
		PrayerInfo:					ChangeND(2,														// Nullify Prayer Curses and upgrade cost
										ChangeND(2,"PrayerInfo","0",[4,6]),
										"None._Even_curses_need_time_off_every_now_and_then.",[2]),
		MTXinfo:					ChangeND(4,
										ChangeND(4,"MTXinfo",0,[3,7]),
										function(t) {return Math.max(t, cheatConfig.wide.gembuylimit);},[5]),// Nullify MTX cost
...
function setupCListProxy() {
	...
	for (const [key, value] of Object.entries(CListFuncDict)) {
		Object.defineProperty(CListAttr, key, {
			get: function () {
				if ((cheatState.cauldron.vialrng && key === "AlchemyVialItemsPCT") ||
...
function setupw3StuffProxy() {
	...
	actorEvents345._customBlock_WorkbenchStuff = function(...argumentsList) {
			...
			if (cheatState.multiply.printer && t == "ExtraPrinting") return (argumentsList[0] = "AdditionExtraPrinting", cheatConfig.multiply.printer * Reflect.apply(Workbench, this, argumentsList)); // print multiplier
...
function setupw4StuffProxy() {
	...
	actorEvents345._customBlock_Labb = function(...argumentsList) {
		if ((cheatState.w4.labpx) 		&& (argumentsList[0] == "Dist" || argumentsList[0] == "BonusLineWidth")) return 1000; 		// long lab connections
	...
	actorEvents345._customBlock_PetStuff = function(...argumentsList) {
		...
		if (cheatState.w4.fastforaging && argumentsList[0] == "TotalTrekkingHR") return cheatConfig.w4.fastforaging(originalValue); 		// fast foraging
		if (cheatState.w4.superpets && cheatConfig.w4.superpets[argumentsList[0]]) return cheatConfig.w4.superpets[argumentsList[0]](originalValue); 	// powerful pets
	...
	actorEvents345._customBlock_CookingR = new Proxy(actorEvents345._customBlock_CookingR, {
		apply: function(originalFn, context, argumentsList) {
			...
			if (cheatState.w4.luckychef 	&& t == "CookingNewRecipeOdds") 	 return 4 == argumentsList[1] ? 1 : 5 == argumentsList[1] ? 0 : Reflect.apply(originalFn, context, argumentsList);		// always cook a new recipe
			if (cheatState.w4.freekitchens 	&& (t == "CookingNewKitchenCoinCost" || t == "CookingUpgSpiceCostQty")) 		return 0; 		// free kitchens and upgrades
	...
	actorEvents345._customBlock_CookingR = function(...argumentsList) {
		...
		if (cheatState.w4.instameals && t == "CookingReqToCook") return 1; 		// super fast food
		if (cheatState.w4.instarecipes && t == "CookingFireREQ") return 1; 		// super fast recipes
		if (cheatState.w4.luckychef && t == "CookingNewRecipeOdds") return 4 == argumentsList[1] ? 1 : 5 == argumentsList[1] ? 0 : Reflect.apply(CookingR, this, argumentsList);		// always cook a new recipe
		if (cheatState.w4.freekitchens && (t == "CookingNewKitchenCoinCost" || t == "CookingUpgSpiceCostQty")) return 0; 		// free kitchens and upgrades
		if (cheatState.w4.freeplates && t == "CookingMenuMealCosts") return 0; //free dinner table upgrades
...
function setupw5Proxies() {
	...
	actorEvents579._customBlock_Sailing = new Proxy(actorEvents579._customBlock_Sailing, {
		apply: function(originalFn, context, argumentsList) {
			if (cheatState.w5.sailing && cheatConfig.w5.sailing.hasOwnProperty(argumentsList[0])) {
				return cheatConfig.w5.sailing[argumentsList[0]](Reflect.apply(originalFn, context, argumentsList));
			}
			return Reflect.apply(originalFn, context, argumentsList);
		}
	});
	...
	actorEvents579._customBlock_GamingStatType = function(...argumentsList) {
		if (cheatState.w5.gaming && cheatConfig.w5.gaming[argumentsList[0]]) {
			return cheatConfig.w5.gaming[argumentsList[0]](Reflect.apply(GamingStatType, this, argumentsList), argumentsList);
		}
	...
	Object.defineProperty(bEngine.getGameAttribute("Gaming"), 13, {
		get: function () {
			return (cheatState.w5.gaming && cheatConfig.w5.gaming.SnailMail ? cheatConfig.w5.gaming.SnailMail : GamingAttr._13);
		},
		set: function (value) {
	...
	actorEvents579._customBlock_AtomCollider = function(...argumentsList) {
		if (cheatState.w5.collider && cheatConfig.w5.collider[argumentsList[0]]) {
			return cheatConfig.w5.collider[argumentsList[0]](Reflect.apply(AtomCollider, this, argumentsList));
		}
	...
	Object.defineProperty(bEngine.getGameAttribute("Rift"), 1, {
		get: function () {
			return (cheatState.unlock.rifts && CList.RiftStuff[4][bEngine.getGameAttribute("Rift")[0]] != 9) ? 1e8 : RiftAttr._1;
		},
		set: function (value) {
			RiftAttr._1 = value;
			return true;
...
function setupPoingProxy() {
	let aiVelocity = 0;
	Object.defineProperty(bEngine.gameAttributes.h.PixelHelperActor[23].behaviors.behaviors[0].script._GenINFO[63], '1', {
		get: function() {
...
function rollPerfectObols(obolOrder, obolMap, characterClass) {
	const primaryStat = [1, 2, 3].includes(characterClass) ? "LUK" : [31, 32, 33, 34, 36].includes(characterClass) ? "WIS" : [7, 8, 9, 10, 12].includes(characterClass) ? "STR" : [19, 20, 21, 22, 25].includes(characterClass) ? "AGI" : "LUK";
	const preferredStat = cheatConfig.wide.perfectobols.preferredstat === 'PRIMARY' ? primaryStat : cheatConfig.wide.perfectobols.preferredstat;
	...
	obolOrder.forEach((obol, index) => {
		...
		if (obolDef["UQ1txt"] != 0) obolMapItem["UQ1txt"] = obolDef["UQ1txt"], obolMapItem["UQ1val"] = 1, rollsLeft--;
		if (obolDef["UQ2txt"] != 0) obolMapItem["UQ2txt"] = obolDef["UQ2txt"], obolMapItem["UQ2val"] = 1, rollsLeft--;
...
function ChangeND(dim, KeyName, repl, elem){
	...
	if(dim === 4){
		...
		for(const [index1, element1] of Object.entries(NDArr)){
			for(const [index2, element2] of Object.entries(element1)){
				for(i in elem) element2[elem[i]] = repl instanceof Function ? repl(element2[elem[i]]) : repl;
			...
			if (typeof obj == "object") {
				...
				for (let index in obj) {
					if (typeof obj[index] == "object") {
						...
						if (which == 0) foundVals.push("  ".repeat(depth)+`${index}: ${obj[index]}`); // This one's for gga
